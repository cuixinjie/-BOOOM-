...
func _load_all_configs() -> void:
	...
	_shop_items = _load_json("res://assets/configs/shop_items.json")
...
func get_game_config(category: String) -> Dictionary:
	# 特殊处理 shop_items，直接返回完整数据
	if category == "shop_items":
		return _shop_items
	# 特殊处理 global_settings，从 level_config 获取
	if category == "global_settings":
		var global_config = _level_config.get("global_settings", {})
		if global_config.is_empty():
			global_config = _game_config.get("global_settings", {})
